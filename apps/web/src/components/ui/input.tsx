  ...
  return (
    <div className="relative w-full">
      ...
      <input
        ...
        // `onChange` comes out alongside `defaultValue`: spread last, it would
        // land back on top of handleInputChange and strand the internal value.
        {...(({ defaultValue: _defaultValue, onChange: _onChange, ...rest }) => rest)(props)}
      />
      ...
      {(rightIcon || isPassword || showClearButton) && (
        <div className="absolute right-3 top-1/2 z-10 flex -translate-y-1/2 items-center gap-1">
          {/* Custom right icon */}
          {rightIcon && (
            <div className="text-muted-foreground [&_svg]:size-4 [&_svg]:shrink-0">{rightIcon}</div>
          )}
